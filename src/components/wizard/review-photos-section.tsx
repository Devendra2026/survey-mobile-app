...
export function ReviewPhotosSection({
  ...
  onEditStep: () => void;
  onFlushReady?: (flush: ReturnType<typeof useWizardPhotoCapture>['flushPhotoQueue']) => void;
}) {
  const {
    photoBySlot,
    previewBySlot,
  ...
  useEffect(() => {
    onFlushReady?.(flushPhotoQueue);
  }, [flushPhotoQueue, onFlushReady]);
      ...
      <View style={{ gap: 12 }} className="mb-3">
        {REQUIRED_SURVEY_PHOTO_SLOTS.map((slot) => (
          <ReviewPhotoCard
            ...
            previewUri={
              previewBySlot[slot] ??
              photoPreviewUri(photoBySlot.get(slot)) ??
